...
export default function Home() {
  ...
  return (
    <main ref={containerRef} className="relative h-screen w-full overflow-hidden bg-black">
      ...
      {/* Content Container */}
      <div className={`absolute bottom-8 left-4 right-4 sm:bottom-12 sm:left-6 sm:right-auto md:bottom-16 md:left-8 lg:bottom-20 lg:left-24 z-10 ${language === 'ar' ? 'left-auto right-4 sm:right-6 md:right-8 lg:right-24 sm:left-auto md:left-auto lg:left-auto' : ''}`}>
        <div className={`max-w-[500px] hero-content ${language === 'ar' ? 'ml-auto mr-0 text-right' : ''}`}>
          ...
          <button 
            ...
            className={`inline-flex items-center px-4 sm:px-6 mt-4 sm:mt-6 rounded-full cursor-pointer py-2 sm:py-3 border-2 border-white text-white hover:bg-white hover:text-black transition-colors duration-300 group text-sm sm:text-base md:text-lg ${language === 'ar' ? 'justify-end' : ''}`}
          >
            {language === 'ar' ? (
              <>
                <span className="font-medium order-last">{t('hero.letsTalk')}</span>
                <ArrowLeft className="mx-2 h-4 w-4 sm:h-5 sm:w-5 transition-transform group-hover:-translate-x-1 order-first" />

              </>
            ) : (
              <>
                <span className="font-medium">{t('hero.letsTalk')}</span>
                <ArrowRight className="mx-2 h-4 w-4 sm:h-5 sm:w-5 transition-transform group-hover:translate-x-1" />
              </>
            )}
          </button>
        </div>
